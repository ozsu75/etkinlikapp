import Etkinlik, { find, countDocuments, findById, findByIdAndDelete } from '../models/Etkinlik';
import { findByIdAndUpdate, findById as _findById } from '../models/User';
import { cloudinary } from '../../config/cloudinary';


// Tüm etkinlikleri listeleme
export async function etkinlikListesi(req, res) {
  try {
    ...
    const etkinlikler = await find(filtre)
    ...
    const toplamEtkinlik = await countDocuments(filtre);
...
export async function etkinlikDetay(req, res) {
  try {
    const etkinlik = await findById(req.params.id)
      .populate('olusturan', 'isim soyisim profilResmi')
...
export function etkinlikOlusturForm(req, res) {
...
export async function etkinlikOlustur(req, res) {
  try {
    // Sadece kurumsal kullanıcılar etkinlik oluşturabilir
    if (req.user.kayitTuru !== 'kurumsal' || !req.user.kurum || !req.user.kurum.onayli) {
      req.flash('error', 'Sadece onaylı kurumsal hesaplar etkinlik oluşturabilir');
...
export async function etkinlikDuzenleForm(req, res) {
  try {
    const etkinlik = await findById(req.params.id);
...
// Etkinlik düzenleme
export async function etkinlikDuzenle(req, res) {
  try {
    const etkinlik = await findById(req.params.id);
    ...
    // Kategorileri diziye çevirme
    const kategoriListesi = kategoriers ? kategoriler.split(',').map(k => k.trim()) : [];
...
// Etkinliğe katılma
export async function etkinligeKatil(req, res) {
  try {
    const etkinlik = await findById(req.params.id);
...
// Etkinlikten ayrılma
export async function etkinliktenAyril(req, res) {
  try {
    const etkinlik = await findById(req.params.id);
...
// Etkinliği beğenme
export async function etkinlikBegen(req, res) {
  try {
    const etkinlik = await findById(req.params.id);
...
export async function etkinlikBegeniKaldir(req, res) {
  try {
    const etkinlik = await findById(req.params.id);
...
export async function etkinlikFavoriEkle(req, res) {
  try {
    const etkinlik = await findById(req.params.id);
...
export async function etkinlikFavoriCikar(req, res) {
  try {
    const etkinlik = await findById(req.params.id);
...
// Yorum ekleme
export async function yorumEkle(req, res) {
  try {
...
// Yorum silme
export async function yorumSil(req, res) {
  try {
...
export async function kullaniciEtkinlikleri(req, res) {
  try {
    const sayfa = parseInt(req.query.sayfa) || 1;
    const limit = parseInt(req.query.limit) || 10;
    const skip = (sayfa - 1) * limit;
    ...
    if (tip === 'olusturduklarim') {
      ...
      const user = await _findById(req.user._id).populate('favoriEtkinlikler');
